...
class BlogUpdate(LoginRequiredMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if obj.author != self.request.user:
            messages.error(request, "PermissionDenied for user: {}!. You only have permission to update your Post".format(self.request.user))
...
class BlogDelete(LoginRequiredMixin, DeleteView):
    ...
    def dispatch(self, request, *args, **kwargs):
        obj = self.get_object()
        if obj.author != self.request.user:
            messages.error(request, "PermissionDenied for user: {}!. You only have permission to Delete your Post".format(self.request.user))
        return super().dispatch(request, *args, **kwargs)
